...
```bash
ISCC.exe installer/BlueMarbleDesktop-x64.iss
ISCC.exe installer/BlueMarbleDesktop-arm64.iss
```
...
src/DesktopEarth/
  ...
  UI/
    Theme.cs                   # Centralized dark/light mode color definitions + P/Invoke
    DarkTabControl.cs          # Custom TabControl with dark mode tab painting
...
- The SettingsForm is one large file (~3400 lines) -- could benefit from being split into partial classes or user controls if it grows further
