...
// ─── Helper to strip API keys from exception text before logging ───
static string SanitizeForLog(string text)
{
    return Regex.Replace(text, @"api_key=[^&\s""'\)]+", "api_key=REDACTED",
        RegexOptions.IgnoreCase);
}

// ─── Single instance check ───
const string mutexName = @"Local\BlueMarbleDesktop-SingleInstance-Mutex";
