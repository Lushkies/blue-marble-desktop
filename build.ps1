...
function Build-Target($rid, $outputDir) {
    ...
    # For ARM64, copy Mesa3D if available (with integrity verification)
    if ($rid -eq "arm64") {
        $mesaSrc = "lib\mesa3d\opengl32.dll"
        $expectedHash = "AF44581538367A8039ABE4B35B80D7B58AD043CF372A87776E61C2D1D261FEC3"
        if (Test-Path $mesaSrc) {
            $actualHash = (Get-FileHash $mesaSrc -Algorithm SHA256).Hash
            if ($actualHash -ne $expectedHash) {
                Write-Host "ERROR: Mesa3D opengl32.dll hash mismatch!" -ForegroundColor Red
                Write-Host "  Expected: $expectedHash" -ForegroundColor Red
                Write-Host "  Actual:   $actualHash" -ForegroundColor Red
                Write-Host "  The DLL may have been tampered with. Re-download from the trusted source." -ForegroundColor Red
                exit 1
            }
            Write-Host "Copying Mesa3D fallback DLL (hash verified)..." -ForegroundColor Yellow
